...
| System prompt | `.sidebar/prompt.md` + mode base + workspace context block | ✅ |
| Tool cwd / paths | `executeTool(..., workspacePath)` | ✅ |
| Chat history | **Persisted** in `.sidebar/state.json` per project | ✅ |
...
| Path | Purpose | Status |
|------|---------|--------|
| `.sidebar/prompt.md` | Extra instructions for agent system prompt | ✅ |
| `.sidebar/tools.json` | Per-project tool rules and custom tool schemas | ✅ |
| `.sidebar/state.json` | Chat sessions, history, editor tab list (autosaved) | ✅ |
...
5. `.sidebar/state.json` restored (chat sessions, open editor tabs)
6. `.sidebar/prompt.md` loaded into system prompt store
7. `.sidebar/tools.json` merged into effective tool policy
