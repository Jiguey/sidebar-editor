<script lang="ts">
  ...
  import { fetchLlamacppModelList, DEFAULT_LLAMACPP_ENDPOINT } from "../llamaCppClient";
  import { toolPolicy as toolPolicyStore } from "../stores/toolPolicy";
  import type { ToolPolicyMode } from "../toolPolicy";
  import { parseWhitelistInput } from "../toolPolicy";
  ...
  type Section = "general" | "models" | "harness" | "appearance";
  ...
  $effect(() => {
    if (!open || chatBackend !== "ollama") return;
  ...
  function handleSave() {
    ...
    if (chatBackend === "ollama") {
      ...
      let next = models.map((m) =>
        m.id === sid ? { ...m, contextWindow: ollamaContextChoice } : m
      );
      if (!next.some((m) => m.id === sid)) {
        const rec = RECOMMENDED_OLLAMA_MODELS.find((r) => r.id === sid);
        if (rec) {
          const max = rec.contextLimitMax ?? rec.contextWindow;
          next.push({
            id: sid,
            name: rec.name,
            provider: "ollama",
            contextWindow: pickContextOption(ollamaContextChoice, max),
            contextLimitMax: max,
          });
        }
      }
          ...
          <div class="stack">
              ...
              <button
                ...
                onclick={() => {
                  ...
                  const allowed = new Set([
                    ...
                    ...RECOMMENDED_OLLAMA_MODELS.map((r) => r.id),
                  ]);
                  if (!allowed.has(selectedModel)) {
                    selectedModel = ollamaModels[0]?.id ?? RECOMMENDED_OLLAMA_MODELS[0]?.id ?? selectedModel;
                  }
                }}
              >
                Ollama (local)
            ...
            <label class="field">
              ...
              {:else if chatBackend === "ollama"}
                <select bind:value={selectedModel} class="input">
                  {#if ollamaModels.length > 0}
                    <optgroup label="Installed (from Ollama)">
                      {#each ollamaModels as model}
                        <option value={model.id}>{model.name}</option>
                      {/each}
                    </optgroup>
                  {/if}
                  <optgroup label="Suggested small models (pull if missing)">
                    <option value="llama3.2:1b">llama3.2:1b — fast, small</option>
                    <option value="qwen2.5:0.5b">qwen2.5:0.5b — very small</option>
                    <option value="tinyllama">tinyllama — matches this app name</option>
                    <option value="gemma2:2b">gemma2:2b — solid tiny</option>
                  </optgroup>
                </select>
              {:else}
